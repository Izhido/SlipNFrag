...
layout(push_constant) uniform Time
{
	...
	layout(offset = 4) int lightmapIndex;
};
...
void main()
{
	...
	uvec4 lowTexEntry = textureLod(fragmentTexture, texCoords, texMip.x);
	uvec4 highTexEntry = textureLod(fragmentTexture, texCoords, texMip.y);
