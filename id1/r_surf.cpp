...
void R_DrawSurfaceBlock8_colored (void);
...
//==============================
	...
	if (r_pixbytes == 1)
	{
		...
		else if ((mt->width & 15) || (mt->height & 15))
		{
...
//==============================
	...
	if (r_pixbytes == 1)
	{
		...
		else if ((mt->width & 15) || (mt->height & 15))
		{
			...
			r_numhblocks = (r_drawsurf.surfwidth >> blockdivshift)*3;
			r_numvblocks = r_drawsurf.surfheight >> blockdivshift;
			pblockdrawer = R_DrawSurfaceBlock8_colored;
		}
		else
		{
...
void R_DrawSurfaceBlock8_colored (void)
{
	...
	// FIXME: use delta rather than both right and left, like ASM?
		...
		int lightright_b = r_lightptr[5];
		r_lightptr += r_lightwidth;
		int lightleftstep_r = ((int)r_lightptr[0] - lightleft_r) >> blockdivshift;
		int lightleftstep_g = ((int)r_lightptr[1] - lightleft_g) >> blockdivshift;
		int lightleftstep_b = ((int)r_lightptr[2] - lightleft_b) >> blockdivshift;
		int lightrightstep_r = ((int)r_lightptr[3] - lightright_r) >> blockdivshift;
		int lightrightstep_g = ((int)r_lightptr[4] - lightright_g) >> blockdivshift;
		int lightrightstep_b = ((int)r_lightptr[5] - lightright_b) >> blockdivshift;
		...
		for (i=0 ; i<blocksize ; i++)
		{
			...
			lighttemp_g = lightleft_g - lightright_g;
			lighttemp_b = lightleft_b - lightright_b;
			lightstep_r = lighttemp_r >> blockdivshift;
			lightstep_g = lighttemp_g >> blockdivshift;
			lightstep_b = lighttemp_b >> blockdivshift;
