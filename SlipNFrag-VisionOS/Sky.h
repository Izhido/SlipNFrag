...
struct Sky
{
	...
	static void Fill(std::vector<SkyEntry>& skyEntries, float*& vertices, uint32_t*& indices, uint32_t& base, id<MTLDevice> device, PerDrawable* perDrawable, std::vector<unsigned char>& skyBuffer, std::unordered_map<void*, NSUInteger>& textureIndex, NSMutableArray<Texture*>* textureCache);
