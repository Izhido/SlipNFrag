...
#include "TurbulentLit.h"
#include "vid_visionos.h"
#include "d_lists.h"
#import "Lightmap.h"

void TurbulentLit::Sort(std::unordered_map<void*, SortedSurfaceLightmap>& sorted, NSUInteger& verticesSize, NSUInteger& indicesSize)
{
	for (auto s = 0; s <= d_lists.last_turbulent_lit; s++)
	{
		auto& turbulent = d_lists.turbulent_lit[s];
		auto face = (msurface_t*)turbulent.face;

		auto& lightmap = sorted[turbulent.face];
		...
		auto& texture = lightmap.textures[turbulent.data];
		...
		for (auto& texture : lightmap.second.textures)
		{
			for (auto s : texture.second.entries)
			{
				...
				*indices++ = base++;
				...
				{
					auto entry = lightmapIndex->find(turbulent.face);
					if (entry == lightmapIndex->end())
					{
						...
						newLightmap.region = MTLRegionMake2D(0, 0, turbulent.lightmap_width, turbulent.lightmap_height);
						
						newLightmap.createdCount = turbulent.created;

						lightmapIndex->insert({turbulent.face, perDrawable.lightmapCache.count});
					...
					else
					{
						...
						if (existingLightmap.createdCount != turbulent.created)
						{
							...
							existingLightmap.createdCount = turbulent.created;
						}
					}
