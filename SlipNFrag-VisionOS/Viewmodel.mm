		...
		for (auto& entry : texture.second.entries)
		{
			...
			if (!texture.second.set)
			{
				...
				if (textureEntry == textureIndex.end())
				{
					...
					[newTexture.texture replaceRegion:newTexture.region mipmapLevel:0 withBytes:viewmodel.data bytesPerRow:newTexture.region.size.width];
			...
			if (viewmodel.colormap != nullptr)
			{
				...
				if (colormapEntry == colormapIndex.end())
				{
					...
					[newColormap.texture replaceRegion:newColormap.region mipmapLevel:0 withBytes:viewmodel.colormap bytesPerRow:newColormap.region.size.width];
