...
void CDAudio_Callback(void* waveHeader)
{
...
void CDAudio_timeCallback(UINT uTimerID, UINT uMsg, DWORD_PTR dwUser, DWORD_PTR dw1, DWORD_PTR dw2)
{
	CDAudio_Callback((void*)dwUser);
}

void CALLBACK CDAudio_waveOutProc(HWAVEOUT hwo, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
	if (uMsg == WOM_DONE)
	{
		UINT delay = 1;
		do
		{
			auto result = timeSetEvent(delay, 0, (LPTIMECALLBACK)CDAudio_timeCallback, dwParam1, TIME_ONESHOT);
			if (result <= 0)
			{
				delay++;
			}
...
void CDAudio_Play(byte track, qboolean looping)
{
	...
	auto result = waveOutOpen(&cdaudio_waveout, deviceId, &cdaudio_waveformat, (DWORD_PTR)CDAudio_waveOutProc, 0, CALLBACK_FUNCTION);
	if (result != MMSYSERR_NOERROR)
	{
		CDAudio_DisposeBuffers();
		return;
...
void CDAudio_Update(void)
{
	...
	if (cdaudio_delaytoobig)
	{
		Sys_Printf("CDAudio_Update: delay for audio callback reached 50ms. Shutting down audio.\n");
