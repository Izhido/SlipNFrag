...
layout(location = 1) in flat vec2 fragmentAttributes;
layout(location = 0) out lowp vec4 outColor;

void main()
{
	if (fragmentAttributes.y > 0)
	{
		ivec2 ditherIndex = ivec2(gl_FragCoord.xy) % 4;
		if (fragmentAttributes.y / 255 < ditherThresholds[ditherIndex.y * 4 + ditherIndex.x])
		{
			discard;
		}
	}
	...
	uvec4 lowColormapped = texelFetch(fragmentColormap, ivec2(lowEntry.x, fragmentAttributes.x), 0);
