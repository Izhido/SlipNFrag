...
layout(set = 4, binding = 0) readonly buffer FragmentLightmap
{
	uint lightmap[];
};
...
layout(location = 3) in flat int fragmentAlpha;
...
void main()
{
	...
	uint lightmapWidth = fragmentSizes.x + 1;
	uint lightmapTopIndex = fragmentIndices.x + int(lightmapClamped.y) * lightmapWidth + int(lightmapClamped.x);
	...
	vec4 glow = mix(lowGlow, highGlow, smoothstep(0.4, 0.6, fract(texLevel.y)));
	vec4 color = mix(lowColor, highColor, smoothstep(0.4, 0.6, fract(texLevel.y))) * vec4(light, light, light, 1) + 2 * glow;
