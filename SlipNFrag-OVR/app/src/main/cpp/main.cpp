...
void createTextureFromAllocation(AppState& appState, VkCommandBuffer commandBuffer, uint32_t width, uint32_t height, VkFormat format, uint32_t mipCount, VkImageUsageFlags usage, TextureFromAllocation*& texture)
{
	texture = new TextureFromAllocation { };
	texture->width = width;
	texture->height = height;
	texture->mipCount = mipCount;
	texture->layerCount = 1;
		...
		if (allocation.allocatedCount < allocation.allocated.size())
		{
			for (auto i = 0; i < allocation.allocated.size(); i++)
			{
				if (!allocation.allocated[i])
				{
					...
					VK(appState.Device.vkBindImageMemory(appState.Device.device, texture->image, allocation.memory, i * allocationList.blockSize));
					texture->allocationList = &allocationList;
					texture->allocationIndex = j;
					texture->allocatedIndex = i;
	...
	if (!found)
	{
		...
		VK(appState.Device.vkBindImageMemory(appState.Device.device, texture->image, allocation.memory, 0));
		texture->allocationList = &allocationList;
		texture->allocationIndex = allocationList.allocations.size() - 1;
		allocation.allocatedCount++;
	}
	VkImageViewCreateInfo imageViewCreateInfo { };
	imageViewCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
...
void deleteTextureFromAllocation(AppState& appState, TextureFromAllocation* texture)
{
	VC(appState.Device.vkDestroyImageView(appState.Device.device, texture->view, nullptr));
	VC(appState.Device.vkDestroyImage(appState.Device.device, texture->image, nullptr));
	auto& allocation = texture->allocationList->allocations[texture->allocationIndex];
	allocation.allocated[texture->allocatedIndex] = false;
	allocation.allocatedCount--;
	if (allocation.allocatedCount <= 0 && texture->allocationIndex == texture->allocationList->allocations.size() - 1)
	{
		VC(appState.Device.vkFreeMemory(appState.Device.device, allocation.memory, nullptr));
		texture->allocationList->allocations.pop_back();
	}
}
	...
	if (oldTextures != nullptr)
	{
		for (TextureFromAllocation** t = oldTextures; *t != nullptr; )
		{
			...
			if ((*t)->unusedCount >= MAX_UNUSED_COUNT)
			{
				...
				deleteTextureFromAllocation(appState, *t);
				delete *t;
				*t = next;
			}
			else
...
void fillMipmappedTextureFromAllocation(AppState& appState, TextureFromAllocation* texture, Buffer* buffer, VkDeviceSize offset, VkCommandBuffer commandBuffer)
{
	VkImageMemoryBarrier imageMemoryBarrier { };
	imageMemoryBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
	imageMemoryBarrier.srcAccessMask = VK_ACCESS_SHADER_READ_BIT;
	...
	while (app->destroyRequested == 0)
	{
		...
		for (auto entry = appState.Scene.surfaces.begin(); entry != appState.Scene.surfaces.end(); )
		{
			...
			if (texture->next == nullptr)
			{
				...
				if (texture->unusedCount >= MAX_UNUSED_COUNT)
				{
					...
					deleteTextureFromAllocation(appState, texture);
					delete texture;
				}
				else
				{
		...
		for (auto& view : appState.Views)
		{
			...
			*((void**)&sceneMatrices) = stagingBuffer->mapped;
			...
			VC(appState.Device.vkCmdPipelineBarrier(perImage.commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, 0, 0, nullptr, 1, &bufferMemoryBarrier, 0, nullptr));
			...
			{
				perImage.host_colormap = new Texture();
				perImage.host_colormap->Create(appState, perImage.commandBuffer, 256, 64, VK_FORMAT_R8_UNORM, 1, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
			...
			for (auto i = 0; i <= d_lists.last_surface; i++)
			{
				...
				if (entry == appState.Scene.surfaces.end())
				{
					...
					createTextureFromAllocation(appState, perImage.commandBuffer, surface.width, surface.height, VK_FORMAT_R8_UNORM, mipCount, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, texture);
			...
			for (auto i = 0; i <= d_lists.last_turbulent; i++)
			{
				...
				else if (perImage.hostClearCount == host_clearcount)
				{
					...
					if (texture == nullptr)
					{
						...
						texture = new Texture();
						texture->Create(appState, perImage.commandBuffer, turbulent.width, turbulent.height, VK_FORMAT_R8_UNORM, mipCount, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
			...
			if (stagingBufferSize > 0)
			{
				...
				if (perImage.host_colormapOffset >= 0)
				{
					perImage.host_colormap->Fill(appState, stagingBuffer, perImage.host_colormapOffset, perImage.commandBuffer);
				}
				for (auto i = 0; i <= d_lists.last_surface; i++)
				{
					if (appState.Scene.surfaceList[i].offset >= 0)
					{
						fillMipmappedTextureFromAllocation(appState, appState.Scene.surfaceList[i].texture, stagingBuffer, appState.Scene.surfaceList[i].offset, perImage.commandBuffer);
					}
				}
				for (auto i = 0; i <= d_lists.last_sprite; i++)
				{
				...
				for (auto i = 0; i <= d_lists.last_turbulent; i++)
				{
					if (appState.Scene.turbulentList[i].offset >= 0)
					{
						appState.Scene.turbulentList[i].texture->FillMipmapped(appState, stagingBuffer, appState.Scene.turbulentList[i].offset, perImage.commandBuffer);
					}
				}
				for (auto i = 0; i <= d_lists.last_alias; i++)
				{
					...
					if (!alias.is_host_colormap)
					{
						...
						{
							texture = perImage.colormaps.oldTextures;
							perImage.colormaps.oldTextures = texture->next;
						}
						else
						{
							texture = new Texture();
				...
				if (d_lists.last_sky >= 0)
				{
					if (perImage.sky == nullptr)
					{
						...
						perImage.sky = new Texture();
						perImage.sky->Create(appState, perImage.commandBuffer, 128, 128, VK_FORMAT_R8_UNORM, mipCount, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
					}
					perImage.sky->FillMipmapped(appState, stagingBuffer, perImage.skyOffset, perImage.commandBuffer);
				}
				if (appState.Mode != AppWorldMode && appState.Scene.floorTexture == nullptr)
				{
					appState.Scene.floorTexture = new Texture();
					appState.Scene.floorTexture->Create(appState, perImage.commandBuffer, appState.FloorWidth, appState.FloorHeight, VK_FORMAT_R8G8B8A8_UNORM, 1, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
					appState.Scene.floorTexture->Fill(appState, stagingBuffer, appState.Scene.floorOffset, perImage.commandBuffer);
				}
			}
			double clearR;
			double clearG;
			...
			if (verticesSize > 0)
			{
				...
				if (!perImage.host_colormapResources.created && perImage.host_colormap != nullptr)
				{
					...
					textureInfo[0].sampler = appState.Scene.samplers[perImage.host_colormap->mipCount];
				...
				if (!perImage.sceneMatricesResources.created || !perImage.sceneMatricesAndPaletteResources.created)
				{
					...
					if (!perImage.sceneMatricesAndPaletteResources.created)
					{
						...
						textureInfo[0].sampler = appState.Scene.samplers[perImage.palette->mipCount];
				...
				if (appState.Mode == AppWorldMode)
				{
					...
					if (d_lists.last_sky >= 0)
					{
						...
						if (!perImage.skyResources.created)
						{
							...
							textureInfo[0].sampler = appState.Scene.samplers[perImage.sky->mipCount];
						...
						VC(appState.Device.vkCmdPushConstants(perImage.commandBuffer, appState.Scene.sky.pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 13 * sizeof(float), &rotation));
