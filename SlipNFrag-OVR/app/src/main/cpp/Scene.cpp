...
void Scene::Create(AppState& appState, VkCommandBufferAllocateInfo& commandBufferAllocateInfo, VkCommandBuffer& setupCommandBuffer, VkCommandBufferBeginInfo& commandBufferBeginInfo, VkSubmitInfo& setupSubmitInfo, Instance& instance, VkFenceCreateInfo& fenceCreateInfo, struct android_app* app)
{
	...
	std::vector<uint32_t> arrowsData;
	arrowsData.resize(appState.ScreenWidth * appState.ScreenHeight, 255 << 24);
	...
	VK(appState.Device.vkMapMemory(appState.Device.device, leftArrowsBuffer.memory, 0, VK_WHOLE_SIZE, 0, &leftArrowsBuffer.mapped));
	memcpy(leftArrowsBuffer.mapped, arrowsData.data(), leftArrowsBuffer.size);
	mappedMemoryRange.memory = leftArrowsBuffer.memory;
	VC(appState.Device.vkFlushMappedMemoryRanges(appState.Device.device, 1, &mappedMemoryRange));
	VC(appState.Device.vkUnmapMemory(appState.Device.device, leftArrowsBuffer.memory));
	leftArrowsBuffer.mapped = nullptr;
	imageMemoryBarrier.srcAccessMask = 0;
	imageMemoryBarrier.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
	imageMemoryBarrier.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED;
	imageMemoryBarrier.newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
	...
	VC(appState.Device.vkCmdPipelineBarrier(setupCommandBuffer, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &imageMemoryBarrier));
	VC(appState.Device.vkCmdCopyBufferToImage(setupCommandBuffer, leftArrowsBuffer.buffer, appState.LeftArrows.Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region));
	...
	VK(appState.Device.vkMapMemory(appState.Device.device, rightArrowsBuffer.memory, 0, VK_WHOLE_SIZE, 0, &rightArrowsBuffer.mapped));
	memcpy(rightArrowsBuffer.mapped, arrowsData.data(), rightArrowsBuffer.size);
	mappedMemoryRange.memory = rightArrowsBuffer.memory;
	VC(appState.Device.vkFlushMappedMemoryRanges(appState.Device.device, 1, &mappedMemoryRange));
	VC(appState.Device.vkUnmapMemory(appState.Device.device, rightArrowsBuffer.memory));
	rightArrowsBuffer.mapped = nullptr;
	imageMemoryBarrier.srcAccessMask = 0;
	imageMemoryBarrier.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
	imageMemoryBarrier.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED;
	imageMemoryBarrier.newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
	...
	VC(appState.Device.vkCmdPipelineBarrier(setupCommandBuffer, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &imageMemoryBarrier));
	VC(appState.Device.vkCmdCopyBufferToImage(setupCommandBuffer, rightArrowsBuffer.buffer, appState.RightArrows.Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region));
	...
	rightArrowsBuffer.Destroy(appState);
	leftArrowsBuffer.Destroy(appState);
