...
void Scene::Create(AppState& appState, VkCommandBufferAllocateInfo& commandBufferAllocateInfo, VkCommandBuffer& setupCommandBuffer, VkCommandBufferBeginInfo& commandBufferBeginInfo, VkSubmitInfo& setupSubmitInfo, Instance& instance, VkFenceCreateInfo& fenceCreateInfo, struct android_app* app)
{
	...
	matrices.size = numBuffers * 2 * sizeof(ovrMatrix4f);
	bufferCreateInfo.size = matrices.size;
	bufferCreateInfo.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
	VK(appState.Device.vkCreateBuffer(appState.Device.device, &bufferCreateInfo, nullptr, &matrices.buffer));
	matrices.flags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
	VC(appState.Device.vkGetBufferMemoryRequirements(appState.Device.device, matrices.buffer, &memoryRequirements));
	createMemoryAllocateInfo(appState, memoryRequirements, matrices.flags, memoryAllocateInfo);
	VK(appState.Device.vkAllocateMemory(appState.Device.device, &memoryAllocateInfo, nullptr, &matrices.memory));
	VK(appState.Device.vkBindBufferMemory(appState.Device.device, matrices.buffer, matrices.memory, 0));
