...
void PerFrame::LoadStagingBuffer(AppState& appState, Buffer* stagingBuffer)
{
	...
	if (appState.Scene.hostColormapSize > 0)
	{
...
void PerFrame::LoadNonStagedResources(AppState &appState)
{
	...
	for (auto buffer : lightmapBuffersToUnmap)
	{
		buffer->buffer.UnmapAndFlush(appState);
		buffer->buffer.mapped = nullptr;
	}
...
void PerFrame::FillFromStagingBuffer(AppState& appState, Buffer* stagingBuffer, uint32_t swapchainImageIndex)
{
	...
	if (appState.Scene.hostColormapSize > 0)
	{
        appState.Scene.hostColormap.Fill(appState, appState.Scene.stagingBuffer);
		appState.Scene.stagingBuffer.offset += appState.Scene.hostColormapSize;
	}
...
void PerFrame::SetTintPushConstants(const AppState& appState, AliasColoredLightsPushConstants& pushConstants)
{
	...
	pushConstants.gamma = appState.FromEngine.vgamma;
}
...
void PerFrame::Render(AppState& appState, uint32_t swapchainImageIndex)
{
	...
	if (!hostColormapResources.created && appState.Scene.hostColormap.image != VK_NULL_HANDLE)
	{
		poolSizes[0].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		poolSizes[0].descriptorCount = 1;
		textureInfo.sampler = appState.Scene.sampler;
		textureInfo.imageView = appState.Scene.hostColormap.view;
		writes[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		writes[0].pImageInfo = &textureInfo;
		descriptorPoolCreateInfo.poolSizeCount = 1;
		CHECK_VKCMD(vkCreateDescriptorPool(appState.Device, &descriptorPoolCreateInfo, nullptr, &hostColormapResources.descriptorPool));
		descriptorSetAllocateInfo.descriptorPool = hostColormapResources.descriptorPool;
		descriptorSetAllocateInfo.pSetLayouts = &appState.Scene.singleImageLayout;
		CHECK_VKCMD(vkAllocateDescriptorSets(appState.Device, &descriptorSetAllocateInfo, &hostColormapResources.descriptorSet));
		writes[0].dstSet = hostColormapResources.descriptorSet;
		...
		if (!sceneMatricesAndPaletteResources.created || (!aliasResources.created && sortedAttributes != nullptr) || !sceneMatricesAndNeutralPaletteResources.created || (!aliasNeutralPaletteResources.created && sortedAttributes != nullptr) || (!sceneMatricesAndColormapResources.created && appState.Scene.hostColormap.image != VK_NULL_HANDLE))
		{
			...
			if (!sceneMatricesAndPaletteResources.created || (!aliasResources.created && sortedAttributes != nullptr))
			{
				...
				if (!aliasResources.created && sortedAttributes != nullptr)
				{
					...
					bufferInfo[2].buffer = sortedAttributes->buffer;
					bufferInfo[2].range = sortedAttributes->size;
	...
	if (appState.Mode == AppWorldMode)
	{
		...
		if (appState.Scene.alias.last >= 0)
		{
			...
			appState.Scene.alias.Render(commandBuffer, hostColormapResources.descriptorSet);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.aliasColoredLights.last >= 0)
		{
			...
			SetTintPushConstants(appState, pushConstants);
			vkCmdPushConstants(commandBuffer, appState.Scene.aliasColoredLights.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(AliasColoredLightsPushConstants), &pushConstants);
			appState.Scene.aliasColoredLights.Render(commandBuffer);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		...
		if (appState.Scene.aliasAlpha.last >= 0)
		{
			...
			appState.Scene.aliasAlpha.Render(commandBuffer, hostColormapResources.descriptorSet);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.aliasHoley.last >= 0)
		{
			...
			appState.Scene.aliasHoley.Render(commandBuffer, hostColormapResources.descriptorSet);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.aliasHoleyAlpha.last >= 0)
		{
			...
			appState.Scene.aliasHoleyAlpha.Render(commandBuffer, hostColormapResources.descriptorSet);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.aliasAlphaColoredLights.last >= 0)
		{
			...
			AliasColoredLightsPushConstants pushConstants { };
			SetTintPushConstants(appState, pushConstants);
			vkCmdPushConstants(commandBuffer, appState.Scene.aliasAlphaColoredLights.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(AliasColoredLightsPushConstants), &pushConstants);
			appState.Scene.aliasAlphaColoredLights.Render(commandBuffer);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.aliasHoleyColoredLights.last >= 0)
		{
			...
			AliasColoredLightsPushConstants pushConstants { };
			SetTintPushConstants(appState, pushConstants);
			vkCmdPushConstants(commandBuffer, appState.Scene.aliasHoleyColoredLights.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(AliasColoredLightsPushConstants), &pushConstants);
			appState.Scene.aliasHoleyColoredLights.Render(commandBuffer);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.aliasHoleyAlphaColoredLights.last >= 0)
		{
			...
			AliasColoredLightsPushConstants pushConstants { };
			SetTintPushConstants(appState, pushConstants);
			vkCmdPushConstants(commandBuffer, appState.Scene.aliasHoleyAlphaColoredLights.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(AliasColoredLightsPushConstants), &pushConstants);
			appState.Scene.aliasHoleyAlphaColoredLights.Render(commandBuffer);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.viewmodels.last >= 0)
		{
			...
			appState.Scene.viewmodels.Render(commandBuffer, hostColormapResources.descriptorSet);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.viewmodelsHoley.last >= 0)
		{
			...
			appState.Scene.viewmodelsHoley.Render(commandBuffer, hostColormapResources.descriptorSet);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.viewmodelsColoredLights.last >= 0)
		{
			...
			AliasColoredLightsPushConstants pushConstants { };
			SetTintPushConstants(appState, pushConstants);
			vkCmdPushConstants(commandBuffer, appState.Scene.viewmodelsColoredLights.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(AliasColoredLightsPushConstants), &pushConstants);
			appState.Scene.viewmodelsColoredLights.Render(commandBuffer);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
		if (appState.Scene.viewmodelsHoleyColoredLights.last >= 0)
		{
			...
			AliasColoredLightsPushConstants pushConstants { };
			SetTintPushConstants(appState, pushConstants);
			vkCmdPushConstants(commandBuffer, appState.Scene.viewmodelsHoleyColoredLights.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(AliasColoredLightsPushConstants), &pushConstants);
			appState.Scene.viewmodelsHoleyColoredLights.Render(commandBuffer);
#if !defined(NDEBUG) || defined(ENABLE_DEBUG_UTILS)
			appState.vkCmdEndDebugUtilsLabelEXT(commandBuffer);
#endif
		}
